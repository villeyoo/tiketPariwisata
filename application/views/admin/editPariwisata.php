...
<body>
        ...
        <div class="vertical-layout">
                ...
                <ul class="nav-menu">
                    ...
                    <li class="nav-menu-item router-link-active">
                        <a href="#">
                            ...
                            <span class="nav-menu-item-title">Edit Pariwisata</span>
                        </a>
                    </li>
                    <!-- <li class="nav-menu-item">
            ...
            <div class="content">
                <div class="main">
                    ...
                    <br>
                    <form action="<?= base_url('tiket/updatePariwisata') ?> " method="POST">
                        <!-- Form di sebelah kiri -->
                        <div class="card">
                            <div class="card-body">
                                <h5>ID</h5>
                                <p style="color:red;">ID tidak bisa di ubah</p>
                                <input type="text" name="id" id="id" style="width:100%;" class="input-text"
                                    placeholder="Masukan Kota Tujuan"
                                    oninput="this.value = this.value.replace(/\b\w/g, l => l.toUpperCase());" readonly
                                    value="<?= $tiket['id'] ?>">
                                </select>
                                <h5>Masukan Tempat Wisata</h5>
                                <p>Wisata sebelumnya
                                    <?= $tiket['Wisata'] ?>
                                </p>
                                <div id="formWisata">
                                    <input type="text" name="wisata[]" style="width:100%;" class="input-text"
                                        placeholder="Masukkan tempat wisata"
                                        oninput="this.value = this.value.replace(/\b\w/g, l => l.toUpperCase());"
                                        required value="<?= $tiket['Wisata'] ?>">
                               
                                </div>
                                <br>
                                <button type="button" onclick="tambahInput()" class="btn btn-danger">Tambah Tempat
                                    Wisata lainnya</button>
                                <br>
                                <br>
                                <br>
                                <h5>Harga masuk Per Orang</h5>
                                <input type="text" id="harga" name="harga" class="input-text1" placeholder="RP" required
                                    value="<?= $tiket['Harga'] ?>">
                                <button type="submit" class="btn btn-primary" style="margin-left:10px;">Perbarui
                                    Data</button>
                            </div>
                        </div>
    ...
    <script>
        ...
        // Event listener untuk memformat input dengan "RP" dan titik sebagai pemisah ribuan
        hargaInput.addEventListener('input', function (event) {
            let formattedValue = event.target.value.replace(/\D/g, ""); // Menghapus karakter non-digit
            formattedValue = formattedValue.replace(/\B(?=(\d{3})+(?!\d))/g, "."); // Menambahkan titik sebagai pemisah ribuan
            event.target.value = "RP " + formattedValue; // Menambahkan "RP" di depan angka yang diformat
        });

    </script>
    <script>
        // Fungsi untuk menampilkan opsi kota-kota berdasarkan provinsi yang dipilih
        function updateCities() {
            var selectedProvince = document.getElementById("provinsi").value;
            var citiesInput = document.getElementById("kota");
            ...
            citiesInput.add(defaultOption);

            // Tambahkan opsi kota berdasarkan provinsi yang dipilih
            if (selectedProvince === "Jawa Barat") {
                var cities = ["Bandung", "Tasikmalaya", "Pangandaran"]; // dan lain-lain
                cities.forEach(function (city) {
                    var option = document.createElement("option");
                    option.value = city;
                    option.text = city;
                    citiesInput.add(option);
                });
            }
            if (selectedProvince === "Jawa Tengah") {
                var cities = ["Semarang", "Solo", "Magelang"]; // dan lain-lain
                cities.forEach(function (city) {
                    var option = document.createElement("option");
                    option.value = city;
                    option.text = city;
            ...
            if (selectedProvince === "DKI Jakarta") {
                var cities = ["Jakarta Pusat", "Jakarta Barat", "Jakarta Selatan"]; // dan lain-lain
                cities.forEach(function (city) {
                    var option = document.createElement("option");
                    option.value = city;
                    option.text = city;
            ...
            // Tambahkan if-else statement untuk provinsi-provinsi lain jika diperlukan
            if (selectedProvince === "Jawa timur") {
                var cities = ["Surabaya"]; // dan lain-lain
                cities.forEach(function (city) {
                    var option = document.createElement("option");
                    option.value = city;
                    option.text = city;
            ...
            if (selectedProvince === "DIY Yogyakarta") {
                var cities = ["Kota Yogyakarta", "Kabupaten Bantul", "Kabupaten Sleman", "Kabupaten Gunung Kidul", "Kabupaten Kulon Progo"];
                cities.forEach(function (city) {
                    var option = document.createElement("option");
                    option.value = city;
                    option.text = city;
        ...
        // Panggil fungsi updateCities() setiap kali pilihan provinsi berubah
        document.getElementById("provinsi").addEventListener("change", updateCities);
    </script>
    <script>
        // Fungsi untuk menampilkan form input setelah memilih nama kota
        function showFormInput() {
            var selectedCity = document.getElementById("kota").value;
            var wisata = document.getElementById("wisata");
            var tulis = document.getElementById("tulis");
            var wst = document.getElementById("wst");

            // Jika nama kota dipilih, tampilkan form input
            if (selectedCity) {
    ...
    <script>
        function tambahInput() {
            ...
            input.placeholder = "Masukkan tempat wisata";
            input.setAttribute("oninput", "this.value = this.value.replace(/\\b\\w/g, l => l.toUpperCase());");
            input.required = true;

            var formWisata = document.getElementById("formWisata");
            formWisata.appendChild(input);
        }
    </script>
